...
/** BFF(API Route)로 요청을 보내고, 실패 시 서버가 내려준 메시지로 ApiError를 throw한다 */
export const bffFetch = async <T>(
  ...
  try {
    ...
    throw new ApiError(fallbackMessage, error);
  }

  if (!response.ok) {
    const body = (await response.json().catch(() => ({}))) as BffErrorResponse;
    throw new ApiError(body.message ?? fallbackMessage, body);
  }
